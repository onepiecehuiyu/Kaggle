...
for e in range(epoch):
	...
	if cv == 0:
		...
		preds = pd.DataFrame(extract_predictions(X_test_ext, bst.predict(xgmat_test)),
							 columns=['Class_' + str(i + 1) for i in range(num_classes)])
		preds['id'] = ids_test
		preds.to_csv('result/' + os.path.splitext(pred_file)[0] + '.epoch' + str(e) + '.csv', index=False)
	# else:
	# 	count = 0
	# 	for train_index, valid_index in skf:
